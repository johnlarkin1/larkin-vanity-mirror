...
export async function GET(request: NextRequest) {
  ...
  try {
    // Parse repo from env var (format: "owner/repo")
    const repoSlug = process.env.BE_RIGHT_BACK_GITHUB_REPO;
    const token = process.env.GITHUB_TOKEN;

    // Fetch PostHog and GitHub Releases in parallel
    const [website, downloads] = await Promise.all([
      fetchPostHogAnalytics("be-right-back", startDate, endDate),
      repoSlug
        ? (() => {
    ...
    console.error("Be Right Back analytics error:", error);
