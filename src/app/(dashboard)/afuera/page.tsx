...
import { Users, Eye, MousePointerClick, Timer, RefreshCw, Globe, Smartphone, UserPlus, Tag } from "lucide-react";
...
import { useAfueraAnalytics } from "@/hooks/use-afuera-analytics";
...
export default function AfueraPage() {
  const { dateRange, setDateRange } = useDateRange();
  const { data, isLoading, isError, error, refetch, isFetching } =
    useAfueraAnalytics({ dateRange });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold sm:text-3xl">Afuera</h1>
          <p className="text-muted-foreground">
            Analytics for afuera.app — NYC social networking
          </p>
        </div>
        <div className="flex items-center gap-2">
          <ExternalLinkButton
            href="https://afuera.app"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Most triggered custom events on afuera.app</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
      ...
      <Card className="border-dashed">
        <CardHeader>
          ...
          <CardDescription>App Store Connect integration coming soon</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center gap-4 py-8 text-muted-foreground">
            <Smartphone className="h-8 w-8" />
            <p className="text-sm">Download tracking will be available once Afuera is published to the App Store</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-dashed">
        <CardHeader>
          ...
          <CardDescription>Signup tracking coming soon</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center gap-4 py-8 text-muted-foreground">
            <UserPlus className="h-8 w-8" />
            <p className="text-sm">Track signups from the marketing website and iOS app</p>
          </div>
        </CardContent>
      </Card>
    </div>
